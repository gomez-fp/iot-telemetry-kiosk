...
def octavefilter(x, fs, fraction=1, order=6, limits=None, show=0, sigbands =0):
    ...
    if sigbands:
        ...
        peak = np.zeros([len(freq)])
        xb = []
        for idx in range(len(freq)):
            sd = signal.resample(x, (int)(len(x) / factor[idx])) #signal.decimate(x, factor[idx])
            y = signal.sosfilt(sos[idx], sd)
            spl[idx] = 20 * np.log10(np.std(y) / 2e-5)
            resampleY = signal.resample_poly(y,factor[idx],1)
            peak[idx] = 20 * np.log10(np.max(np.abs(resampleY))/2e-5)
            xb.append(resampleY)
        return spl, freq, xb, peak
    else:
        # Create array with SPL for each frequency band
        spl = np.zeros([len(freq)])
        for idx in range(len(freq)):
